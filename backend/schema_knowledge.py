...
SCHEMA_KNOWLEDGE = {
    ...
    "business_rules": [
        ...
        "彩蛋任务：'彩蛋任务ID'>0 表示命中分母（符合彩蛋疾病条件），'是否分子1=是(发分)'=1 才是真正命中（店员提及且传递了关键信息）",
        "彩蛋命中率 = COUNT(彩蛋任务ID>0 AND 是否分子1=是(发分)=1) / COUNT(彩蛋任务ID>0)",
    ],
    "query_tips": [
        ...
        "彩蛋分析：查询彩蛋场景时用 `彩蛋任务ID`>0 过滤分母场景，`是否分子1=是(发分)`=1 过滤分子命中场景",
